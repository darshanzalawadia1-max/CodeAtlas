...
export function mapIncident(dbInc: Database['public']['Tables']['incidents']['Row']): Incident {
  ...
  // Read AI recommendations from the backend ML pipeline (if available)
  const aiRec = (dbInc as any).ai_recommendation || "AI assessment pending from ingestion pipeline.";
  const recommendedUnits: string[] = (dbInc as any).recommended_units || [];
  const sourceEvents: string[] = (dbInc as any).source_events || [];
  ...
  return {
    ...
    timeReceived,
    timeAgo,
    assignedUnits: recommendedUnits,
    status: fromDBStatus(dbInc.status),
    callerReport: dbInc.description || "No description provided.",
    aiRecommendation: aiRec,
    duplicateReportsCount: dbInc.report_count || sourceEvents.length || 1,
    timeline: [
      {
        time: timeReceived,
...
export function mapResource(dbRes: Database['public']['Tables']['resources']['Row']): ResponseUnit {
  // Read live lat/lng pushed by backend telemetry (falls back to Ahmedabad center)
  const lat = (dbRes as any).latitude ?? 23.0225;
  const lng = (dbRes as any).longitude ?? 72.5714;
