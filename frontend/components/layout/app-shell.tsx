...
import { ingestEmergencyCall } from "@/lib/api";
...
export const AppShell: React.FC = () => {
  ...
  // ACTIONS
  // 1. Create Incident — Routes through the FastAPI ML Pipeline for real classification.
  //    Falls back to direct Supabase insert if the backend is offline.
  const handleCreateIncident = async (newIncident: Incident) => {
    let mlSuccess = false;

    // Try the ML Pipeline first
    try {
      const result = await ingestEmergencyCall({
        transcript: newIncident.callerReport,
        lat: newIncident.coordinates.lat,
        lng: newIncident.coordinates.lon,
        caller_id: `web-${Date.now()}`,
      });

      if (result.status === "success" && result.incident) {
        mlSuccess = true;
        // The backend already wrote the incident to Supabase.
        // The Realtime subscription (incidentSub) will automatically pick it up
        // and add it to our incidents state — so we do NOT setIncidents here.

        // Create an alert for the activity feed
        await supabase.from("alerts").insert({
          incident_id: result.incident.id,
          alert_type: "dispatch",
          message: `ML Pipeline classified: ${result.incident.incident_type} (${result.incident.severity}). ${result.incident.ai_recommendation || ''}`,
          status: "active"
        });
        ...
        const newAct: ActivityEvent = {
          id: `act-${Date.now()}`,
          timestamp: nowStr,
          title: `Incident created via ML Pipeline`,
          description: `XGBoost classified as ${result.incident.incident_type} (${result.incident.severity}). ${result.incident.recommended_units?.length ? 'Recommended: ' + result.incident.recommended_units.join(', ') : ''}`,
          type: "dispatch",
          severity: newIncident.severity,
          relatedIncidentId: result.incident.id,
        };
        setActivities((prev) => [newAct, ...prev]);

        playAlertTone("create");
        addToast("success", `Incident Classified by ML`, `Type: ${result.incident.incident_type}, Severity: ${result.incident.severity}`);
      }
    } catch (err) {
      console.warn("[AppShell] ML Pipeline unavailable, falling back to direct Supabase insert:", err);
    }

    // Fallback: If the ML pipeline was unreachable, insert directly into Supabase
    if (!mlSuccess) {
